...
class Level:
    def __init__(self, game):
        ...
        self.large_box_sensors = [0, 1, 2, 3, 4]
        self.small_box_sensors = [5, 6, 7, 8, 9]

        # NEW STANDARD mapping
        self.large_box_color_map = [0, 1, 4, 2, 3]
        self.large_box_symbol_map = [1, 2, 0, 3, 4]

        self.small_box_color_map = [2, 1, 0, 4, 3]
        self.small_box_symbol_map = [2, 1, 4, 0, 3]
# NEW STANDARD, JKP (legacy comment)
    ...
    def level_specific_start_game(self):
        ...
        try:
            self._gui.set_text('Info', f"Maximum points is {self._safe_get_param('maxPoints', 100)}!")
        except Exception:
            pass
    ...
    def level_specific_sensor_value_changed(self, board_id, value_type, sensor_id, sensor_value):
        ...
        if self.flag_sensor[sensor_id_int]:
            # already pressed / locked until next generation
            return do_only_level_specific

        self.flag_sensor[sensor_id_int] = True
    ...
    def _light_all_buttons(self):
        """
        Legacy game lit all buttons using SDP commands.
        Here we light sensor IDs 0-9 to a neutral white-ish color as a sane default.
        """
        if not self._arduinos:
            return
        a = self._arduinos[0]

        sensor_ids = sorted(set(self.large_box_sensors + self.small_box_sensors))

        # Prefer send_rgb_values if available
        if hasattr(a, "send_rgb_values"):
            payload = []
            for sid in sensor_ids:
                payload.append((sid, 255, 255, 255))
            # Try a few possible signatures
            for args, kwargs in [
                ((payload,), {}),
                ((), {"values": payload}),
                ((sensor_ids, [255]*len(sensor_ids), [255]*len(sensor_ids), [255]*len(sensor_ids)), {}),
            ]:
                try:
                    a.send_rgb_values(*args, **kwargs)
                    return
                except Exception:
                    continue

        # Fallback to older API used in some reference levels
        if hasattr(a, "send_led_values_to_arduino"):
            for sid in sensor_ids:
                try:
                    a.send_led_values_to_arduino(my_id=sid, color_id=5)
                except Exception:
                    pass
